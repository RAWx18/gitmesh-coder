...
class GitRepo:
    repo = None
    cosmos_ignore_file = None
    cosmos_ignore_spec = None
    cosmos_ignore_ts = 0
    cosmos_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def commit(self, fnames=None, context=None, message=None, cosmos_edits=False, coder=None):
        ...
        Args:
            ...
            cosmos_edits (bool, optional): Whether the changes were made by cosmos. Defaults to False.
        ...
        Cosmos generated the changes (`cosmos_edits`).
        ...
        - cosmos_edits=True: Changes were generated by Cosmos (LLM).
        - cosmos_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (cosmos)".
        - --attribute-committer: Modify Committer name to "User Name (cosmos)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: cosmos (<model>) <cosmos@cosmos.chat>" trailer to commit message.

        Behavior Summary:

        1. When cosmos_edits = True (AI Changes):
        ...
        2. When cosmos_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as Cosmos runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(cosmos),
          Committer=You(cosmos)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(cosmos), Committer=You, Trailer added
        - User commit (defaults): cosmos_edits=False -> Author=You, Committer=You(cosmos)
        - User commit with explicit no-committer: cosmos_edits=False,
        ...
        prefix_commit_message = cosmos_edits and (
        ...
        if cosmos_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: cosmos ({model_name}) <cosmos@cosmos.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to cosmos edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            cosmos_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of cosmos_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an cosmos edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (cosmos_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "cosmos: " + commit_message
        ...
        committer_name = f"{original_user_name} (cosmos)"
    ...
    def refresh_cosmos_ignore(self):
        if not self.cosmos_ignore_file:
            return

        current_time = time.time()
        if current_time - self.cosmos_ignore_last_check < 1:
            return

        self.cosmos_ignore_last_check = current_time

        if not self.cosmos_ignore_file.is_file():
            return

        mtime = self.cosmos_ignore_file.stat().st_mtime
        if mtime != self.cosmos_ignore_ts:
            self.cosmos_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.cosmos_ignore_file.read_text().splitlines()
            self.cosmos_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.cosmos_ignore_file or not self.cosmos_ignore_file.is_file():
            return False
        ...
        return self.cosmos_ignore_spec.match_file(fname)
