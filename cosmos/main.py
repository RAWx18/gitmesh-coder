...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run cosmos in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track cosmos's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".cosmos"):
            patterns_to_add.append(".cosmos*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the cosmos browser feature",
        ["cosmos-chat[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Cosmos-AI/cosmos/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Cosmos-AI/cosmos/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".cosmos.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading cosmos model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".cosmos" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("cosmos.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".cosmos.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Cosmos only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run cosmos --no-git to proceed without using git.")
        return False
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".cosmos.conf.yml")
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Cosmos respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve cosmos?"
            )
    ...
    if args.cosmosignore:
        ignores.append(args.cosmosignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from cosmos.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".cosmos" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install cosmos properly?")
                sys.exit(1)
