...
VERSION_CHECK_FNAME = Path.home() / ".cosmos" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of cosmos from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of cosmos from the main branch?",
        ["git+https://github.com/Cosmos-AI/cosmos.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of cosmos from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer cosmos version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of cosmos?"

    docker_image = os.environ.get("COSMOS_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    if success:
        io.tool_output("Re-run cosmos to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/cosmos-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = cosmos.__version__
