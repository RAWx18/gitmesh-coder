...
from cosmos.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from cosmos.deprecated import add_deprecated_model_args

from .dump import dump  # noqa: F401


def resolve_cosmosignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="cosmos is AI pair programming in your terminal",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="COSMOS_",
    )
    ...
    from cosmos import coders as _cosmos_coders

    edit_format_choices = sorted(
        {
            c.edit_format
            for c in _cosmos_coders.__all__
    ...
    group.add_argument(
        ...
        default=".cosmos.model.settings.yml",
        help="Specify a file with cosmos model settings for unknown models",
    ).complete = shtab.FILE
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".cosmos.model.metadata.json",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .cosmos.llm.history)",
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .cosmos* to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable the addition of files listed in .gitignore to Cosmos's editing scope.",
    )
    default_cosmosignore_file = (
        os.path.join(git_root, ".cosmosignore") if git_root else ".cosmosignore"
    )

    group.add_argument(
        "--cosmosignore",
        metavar="COSMOSIGNORE",
        type=lambda path_str: resolve_cosmosignore_path(path_str, git_root),
        default=default_cosmosignore_file,
        help="Specify the cosmos ignore file (default: .cosmosignore in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute cosmos code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute cosmos commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for cosmos edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'cosmos: ' if cosmos authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'cosmos: ' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            "Attribute cosmos edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new cosmos versions on launch",
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade cosmos to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help="Run cosmos in your browser (default: False)",
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between cosmos and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .cosmos.conf.yml in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: cosmos --shell-completions bash"
        ),
    )
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = "cosmos"  # Set the program name on the parser
