...
class InputOutput:
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# cosmos chat started at {current_time}\n\n")
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'Cosmos' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "Cosmos"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'Cosmos' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
