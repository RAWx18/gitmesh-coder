...
from cosmos import __version__, models, prompts, utils
from cosmos.analytics import Analytics
from cosmos.commands import Commands
from cosmos.exceptions import LiteLLMExceptions
from cosmos.history import ChatSummary
from cosmos.io import ConfirmGroup, InputOutput
from cosmos.linter import Linter
from cosmos.llm import litellm
from cosmos.models import RETRY_TIMEOUT
from cosmos.reasoning_tags import (
    REASONING_TAG,
...
class Coder:
    ...
    repo = None
    last_cosmos_commit_hash = None
    cosmos_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import cosmos.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                cosmos_commit_hashes=from_coder.cosmos_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"Cosmos v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .cosmosignore"
                )
    ...
    def __init__(
        ...
        if cosmos_commit_hashes:
            self.cosmos_commit_hashes = cosmos_commit_hashes
        else:
            self.cosmos_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches cosmosignore spec.")
                continue
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("COSMOS_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.cosmos_edited_files.update(edited)
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_cosmos_commit_hash = commit_hash
        self.cosmos_commit_hashes.add(commit_hash)
        self.last_cosmos_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each cosmos commit.")
